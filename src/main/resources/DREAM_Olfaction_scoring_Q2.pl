...
print STDERR "Script ASSUMES that all the files to be evaluated passed the validation script\n";
#read Gold Standard
...
($ref_gs_val,$ref_gs_sigma) = &read_GS($gs_fn);
    ...
    my ($ref_pred_val, $ref_pred_sigma) = &read_prediction("$dir/$in_fn");
    ...
    open (FH, "> $dir/$out_fn") or die "cannot open $dir/$out_fn (why? $!)\n"; 
